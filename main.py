...
from model.model import MVATModel
import torch
...
parser = argparse.ArgumentParser(description="Set random seed for training.")
...
parser.add_argument('--epochs', type=int, default=28,nargs='?', help='display an integer')
parser.add_argument('--alpha', type=float, default=0.3,nargs='?', help='display a float')
parser.add_argument('--beta', type=float, default=0.9,nargs='?', help='display a float')
...
parser.add_argument('--random_seed', type=int, default=None, nargs='?', help="Random seed for the training")
...
def get_random_seed():
    ...
    return int(time.time() * 1000) % (2 ** 32)  # Using current time in milliseconds as seed


def set_random_seed(random_seed=None):
    """Set random seed"""
    if random_seed is None:
        # If no seed is provided, generate one based on the current time
    ...
    # Optionally, disable deterministic mode to get non-reproducible results
    torch.backends.cudnn.deterministic = False
    torch.backends.cudnn.benchmark = True  # Enable for non-deterministic operations on GPUs (e.g., convolution)
# Example of how you might use argparse to handle command line argument for random seed
...
data_input_file = os.path.join('D:/pytorch_project/DTCA-main/datasets/finetune/dualc/2015/input.pt')
...
if text_model_name == 'bert':
    model_path1 = '/root/autodl-tmp/bert-base-uncased/'
    ...
    model_path1 = "D:/pytorch_project/DTCA-main/roberta-base"
    config1 = AutoConfig.from_pretrained(model_path1)
...
if image_model_name == 'vit':
    model_path2 = 'D:/pytorch_project/DTCA-main/vit-base-patch16-224-in21k'
...
vb_model = MVATModel(config1,config2,aspect_num_labels=5,text_model_name=text_model_name,image_model_name=image_model_name,alpha=alpha,beta=beta)
